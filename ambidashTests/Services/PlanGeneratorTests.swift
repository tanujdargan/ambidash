...
@Test func planGeneratorCreatesActionsForActiveGoals() {
    let goals = [
        Goal(title: "Lean Body", domain: .body, priority: 1),
        Goal(title: "SWE Skills", domain: .craft, priority: 2),
        Goal(title: "Language", domain: .mind, priority: 3),
    ]
...
@Test func planGeneratorRespectsMaxActions() {
    let goals = (1...10).map { Goal(title: "Goal \($0)", domain: .body, priority: $0) }
...
@Test func planGeneratorPrioritizesNeglectedGoals() {
    let fresh = Goal(title: "Fresh", domain: .body, priority: 1)
    fresh.lastProgressDate = .now

    let neglected = Goal(title: "Neglected", domain: .craft, priority: 2)
